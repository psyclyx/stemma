...
test "AnchorSet: sorted shift matches per-anchor shift" {
    ...
    for (0..200) |_| {
        ...
        try handles.append(gpa, try set.add(gpa, a));
    }
...
test "AnchorSet: handles stable across add/remove/compact; range queries" {
    ...
    const h10 = try set.add(gpa, .{ .offset = 10 });
    const h20 = try set.add(gpa, .{ .offset = 20 });
    const h30 = try set.add(gpa, .{ .offset = 30, .bias = .left });
    const h40 = try set.add(gpa, .{ .offset = 40 });
    try t.expectEqual(@as(usize, 4), set.count());

    set.remove(h20);
    set.remove(h40); // triggers compaction (dead > half)
    try t.expectEqual(@as(usize, 2), set.count());
    try t.expectEqual(@as(usize, 10), set.get(h10).offset);
    try t.expectEqual(@as(usize, 30), set.get(h30).offset);

    // Recycled slots don't disturb existing handles.
    const h15 = try set.add(gpa, .{ .offset = 15 });
    try t.expectEqual(@as(usize, 30), set.get(h30).offset);

    var it = set.inRange(.{ .start = 12, .end = 31 });
    ...
    set.set(h15, .{ .offset = 99 });
    var it2 = set.inRange(.{ .start = 0, .end = 1000 });
